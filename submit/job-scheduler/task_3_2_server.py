...
class JobServer:
    def __init__(self):
        # Dictionary of queues for each timestamp
        self.job_queues: Dict[str, Queue] = defaultdict(Queue)
        self.load_jobs()
            ...
            if filename.endswith(".job"):
                ...
                with open(filepath, "r") as f:
                    ...
                    if job_id is not None and created and priority and tasks:
                        # Add job to the queue for its timestamp
                        job = Job(job_id, created, priority, tasks)
                        self.job_queues[created].put(job)

    def get_jobs_by_timestamp(self, timestamp: str) -> Optional[Job]:
        """Return and remove job information for the given timestamp."""
        queue = self.job_queues.get(timestamp)
        if queue and not queue.empty():
            return queue.get()  # Pop and return the next job for this timestamp
        return None
