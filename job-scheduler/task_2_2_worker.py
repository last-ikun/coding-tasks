...
class JobWorker:
    ...
    def _get_sorted_jobs(self):
        """Sort jobs by priority (High before Low)."""
        return sorted(
            list(self.active_tasks.items()),
            key=lambda x: self.job_priorities[x[0]] != "High",  # High priority first
        )
    ...
    def process_tasks(self):
        ...
        for job_id, tasks in self._get_sorted_jobs():
            if not tasks:
...
def main():
    ...
    worker.save_execution_chart(filename="plot/task_2_2_execution_chart.md")
